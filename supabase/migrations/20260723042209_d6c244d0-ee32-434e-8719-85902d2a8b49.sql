...
BEGIN
  ...
  -- Enfileira cupons de impressão para pedidos offline (dinheiro/fiado/maquineta).
  -- Os order_items já foram inseridos acima; o UPDATE dispara o ramo (a) do
  -- trigger trg_orders_enqueue_print, que enfileira com itens presentes.
  -- Pedidos aguardando pagamento (PIX/cartão online) só imprimem via webhook.
  IF NOT v_aguardando_pagamento THEN
...
BEGIN
  ...
  -- AFTER INSERT em orders é no-op: nesse instante order_items ainda não existem.
  -- O create_order faz UPDATE impresso_cozinha=true no final (para pedidos offline),
  -- disparando o ramo (a) abaixo com os itens já gravados.
  IF TG_OP = 'INSERT' THEN
    RETURN NEW;
  END IF;

  -- (a) Comanda de mesa/Caixa OU finalização do create_order (offline):
  -- impresso_cozinha false -> true. NÃO faz UPDATE interno, então não recursa.
