-- =========================================================================
-- FASE 2 — Motor de estoque rígido
-- =========================================================================

-- 1) BAIXA REAL NA VENDA (variação + adicional + conversão + trava controlado)
...
BEGIN
  ...
  LOOP
    IF v_item.manipulado = false THEN
      -- Item de revenda: baixa direta no próprio produto.
    ...
    -- Resolve a variação escolhida (price option) pelo rótulo salvo em size.
    v_po := NULL;
    ...
    -- Ingredientes da BASE (price_option_id IS NULL) + os da variação escolhida.
    FOR v_ing IN
    ...
    LOOP
      -- Pula ingredientes que o cliente pediu para remover.
      ...
      IF v_ing.insumo_id IS NOT NULL THEN
        ...
        IF FOUND THEN
          v_deduct := (v_ing.quantidade * v_item.quantity) / NULLIF(v_fator, 0);
    ...
    -- Adicionais escolhidos (pagos + gratuitos) consomem o insumo vinculado.
      ...
      LOOP
        ...
        LOOP
          SELECT fator_conversao, controlado, nome
            ...
            FROM public.insumos WHERE id = v_ing.insumo_id AND estocavel = true;
          IF FOUND THEN
            v_deduct := (v_ing.quantidade * v_addon_qtd * v_item.quantity) / NULLIF(v_fator, 0);
            UPDATE public.insumos
              SET saldo_estoque = saldo_estoque - v_deduct, updated_at = now()
              WHERE id = v_ing.insumo_id
              RETURNING saldo_estoque, controlado, nome INTO v_saldo, v_controlado, v_nome;
...
-- 2) ESTORNO ESPELHADO (mesma lógica, somando de volta, sem trava)
    ...
    LOOP
      ...
      IF v_ing.insumo_id IS NOT NULL THEN
        SELECT fator_conversao INTO v_fator
          FROM public.insumos WHERE id = v_ing.insumo_id AND estocavel = true;
        IF FOUND THEN
          v_add := (v_ing.quantidade * v_item.quantity) / NULLIF(v_fator, 0);
          UPDATE public.insumos
      ...
      LOOP
        ...
        LOOP
          SELECT fator_conversao INTO v_fator
            FROM public.insumos WHERE id = v_ing.insumo_id AND estocavel = true;
          IF FOUND THEN
            v_add := (v_ing.quantidade * v_addon_qtd * v_item.quantity) / NULLIF(v_fator, 0);
            UPDATE public.insumos
              SET saldo_estoque = saldo_estoque + v_add, updated_at = now()
              WHERE id = v_ing.insumo_id;
          END IF;
...
-- Blindagem: funções internas de baixa/estorno não são chamáveis por API pública.
REVOKE EXECUTE ON FUNCTION public.explode_order_stock(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.explode_order_stock(uuid) TO service_role;
REVOKE EXECUTE ON FUNCTION public.reverse_order_stock(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.reverse_order_stock(uuid) TO service_role;

-- 3) DISPONIBILIDADE PARA O CARDÁPIO (sinal de "Esgotado" preventivo)
    ...
    CASE
      ...
      ELSE EXISTS (
        ...
        WHERE ip.product_id = p.id
          ...
          AND i.saldo_estoque < (ip.quantidade / NULLIF(i.fator_conversao, 0))
      )
    END AS esgotado
...
REVOKE EXECUTE ON FUNCTION public.get_menu_availability() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_menu_availability() TO anon, authenticated, service_role;