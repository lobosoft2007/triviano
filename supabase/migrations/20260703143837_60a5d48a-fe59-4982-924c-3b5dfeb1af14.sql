-- =========================================================================
-- PART A: Lock down self-modifiable financial/authorization columns on profiles
-- =========================================================================
-- Revoke the broad UPDATE grant that let users overwrite ANY column in their
-- own row (including saldo_cashback, fiado_autorizado, limite_fiado,
-- saldo_devedor_fiado and bloqueado).
REVOKE UPDATE ON public.profiles FROM authenticated;

-- Grant UPDATE only on safe, non-financial contact/address columns. Financial
-- and authorization fields are mutated exclusively via SECURITY DEFINER
-- functions that check the admin role (or dedicated wallet/fiado routines).
GRANT UPDATE (
  full_name, phone, address,
  tipo_logradouro, logradouro, numero, complemento, bairro, municipio, estado,
  cep, ddd, telefone, latitude, longitude, push_token
) ON public.profiles TO authenticated;

-- =========================================================================
-- PART B: Server-side order creation with canonical, recomputed pricing
-- =========================================================================
...
-- Restrict execution: authenticated customers only (never anon/public).
REVOKE EXECUTE ON FUNCTION public.create_order(jsonb, text, text, text, text, integer, numeric) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.create_order(jsonb, text, text, text, text, integer, numeric) FROM anon;
GRANT EXECUTE ON FUNCTION public.create_order(jsonb, text, text, text, text, integer, numeric) TO authenticated;