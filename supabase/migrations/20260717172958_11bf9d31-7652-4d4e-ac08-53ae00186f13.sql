...
CREATE OR REPLACE FUNCTION public.admin_get_empresa_config()
 RETURNS TABLE(id uuid, nome_fantasia text, logotipo_url text, taxa_servico_mesa numeric, taxa_entrega_valor numeric, dominio_customizado text, cnpj text, inscricao_estadual text, regime_tributario text, cep text, logradouro text, numero text, complemento text, bairro text, cidade text, estado text, ativo boolean, cor_primaria text, cor_secundaria text, modo_fundo text, percentual_cashback numeric, cashback_ativo boolean, monitor_cozinha boolean, monitor_bar boolean, monitor_pizzaria boolean, ai_report_model text, markup_ifood_percentual numeric)
 LANGUAGE plpgsql
