...
BEGIN
  ...
  IF v_tipo IN ('pix', 'cartao_credito_online', 'cartao_debito_online', 'online')
     AND COALESCE(NEW.pago_online, false) = false THEN
    IF COALESCE(NEW.aguardando_pagamento, true) = false THEN
...
UPDATE public.orders
SET status = 'rascunho_pagamento',
    aguardando_pagamento = true
WHERE tipo_pagamento IN ('pix', 'cartao_credito_online', 'cartao_debito_online', 'online')
  AND COALESCE(pago_online, false) = false
  AND aguardando_pagamento = true;