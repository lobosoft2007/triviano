DROP FUNCTION IF EXISTS public.create_printer_agent_token(text);

CREATE FUNCTION public.create_printer_agent_token(nome text)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  v_empresa_id uuid;
BEGIN
  SELECT pat.empresa_id INTO v_empresa_id
  FROM public.printer_agent_tokens pat
  WHERE pat.id = p_id;
  ...
  UPDATE public.printer_agent_tokens pat
  SET ativo = false
  WHERE pat.id = p_id;
END;
$$;
