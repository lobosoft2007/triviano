-- ============================================================
-- Correção de segurança financeira: pedidos com pagamento ONLINE
-- (PIX/cartão via Mercado Pago) nascem OCULTOS do Caixa/KDS
-- (aguardando_pagamento = true) e só aparecem quando o webhook
-- confirmar o pagamento. Fecha a janela onde o pedido aparecia
-- na cozinha antes do pagamento.
-- ============================================================

DROP FUNCTION IF EXISTS public.create_order(jsonb, text, text, text, text, integer, numeric, text);
...
BEGIN
  ...
  INSERT INTO public.orders (
    ...
    tipo_atendimento, numero_mesa, empresa_id, aguardando_pagamento
  ) VALUES (
    ...
    v_empresa, COALESCE(p_pagamento_online, false)
...
GRANT EXECUTE ON FUNCTION public.create_order(jsonb, text, text, text, text, integer, numeric, text, boolean) TO authenticated, service_role;