...
export const Route = createFileRoute('/api/public/tap/reconcile')({
  server: { handlers: { GET: async ({ request }) => {
    ...
    if (!deviceId || !token || !dia) return new Response(JSON.stringify({ error: 'missing_params' }), { status: 400 })

    const { supabaseAdmin } = await import('@/integrations/supabase/client.server')
    const { data: dev } = await supabaseAdmin.rpc('verify_pos_device', { p_device: deviceId, p_token: token })
    ...
    const { data, error } = await supabaseAdmin.rpc('tap_daily_reconciliation', {
      _empresa_id: empresa_id, _dia: dia,
    } as never)
